...
- First-party SpeedListen now builds local time-compressed audio intake plans from graph-aligned transcripts/recaps, clamps rate by cognitive load, plays through the same session speech contract, and only advances local graph state when comprehension, retention, strain, and distraction gates pass.
