...
- First-party worker service now provides an executable `npm run worker:start` process with env-based memory/Postgres bootstrap, object storage, queue filters, batch/loop modes, scheduler/audio/notification/local-AI/moderation/analytics/export handlers, graceful shutdown, and smoke-tested artifact persistence.
