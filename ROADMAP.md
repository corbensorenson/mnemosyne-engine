...
- PWA Admin surface includes incident command metrics, local incident artifact previews, queued backend-compatible `POST /api/ops/incidents/reports` replay, Privacy Ops cards that queue `POST /api/privacy/export/jobs` plus scoped `DELETE /api/privacy/data` replay for voice and health deletion, and account deletion confirmation alongside the service map and audit log.
