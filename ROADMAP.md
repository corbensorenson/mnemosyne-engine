...
- PWA Today, Morning Forge, GraphFeed, WalkMode, Evening Lock-In, and Sleep now consume bootstrapped persisted goals, daily packets, and sleep audio plans as the primary plan, using local scheduler output only as a fallback.
...
- PWA Morning Forge answer sync now queues backend-compatible raw prompt/answer payloads and posts them directly to `POST /api/morning-forge/complete` when API sync is configured, so scoring, graph updates, session completion, and audit trails happen in the first-party backend.
...
- First-party `@mnemosyne/media-core` now builds lock-screen Media Session plans for WalkMode, Paced Read, and SleepCue, letting the PWA route supported browser media controls back to local learning actions without exposing answers, transcripts, or private graph state.
- First-party `@mnemosyne/audio-core` now builds session speech plans for Morning Forge, Tutor, WalkMode, Evening Lock-In, and SleepCue preview, with local browser speech execution in the PWA, raw-answer/transcript exclusion flags, and quiet fallback text instead of a hosted speech API dependency.
- First-party SpeedListen now builds local time-compressed audio intake plans from graph-aligned transcripts/recaps, clamps rate by cognitive load, plays through the same session speech contract, queues backend-compatible completion evidence, and records replayable `speed_listen_completed` events only after the API recomputes comprehension, retention, strain, and distraction gates.
- First-party accessibility gates now include low-screen audio criteria for speech play controls, immediate stop controls, quiet-environment fallback text, and audio privacy/transcript controls, so audio-first workflows cannot promote as inaccessible.
...
- Accessibility release gates now expose `GET /api/accessibility/release-gate`, auditing keyboard, focus, screen-reader labels, icon labels, reduced motion, contrast, text scaling, phone-overflow, local speech controls, speech stop controls, quiet fallback, and audio privacy checks across the PWA surface inventory.
...
- Production accessibility and reliability gates now include the first-party Tutor surface and `POST /api/tutor/turn` journey, so tutor progress cannot ship outside the release-readiness inventory.
- PWA Admin surface includes incident command metrics, local incident artifact previews, queued backend-compatible `POST /api/ops/incidents/reports` replay, Privacy Ops cards that queue `POST /api/privacy/export/jobs` plus scoped `DELETE /api/privacy/data` replay for voice, health, and confirmation-gated account deletion alongside the service map and audit log.
