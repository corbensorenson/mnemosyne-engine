...
- First-party worker service now provides an executable `npm run worker:start` process with env-based memory/Postgres bootstrap, object storage, queue filters, batch/loop modes, scheduler/audio/analytics/export handlers, graceful shutdown, and smoke-tested artifact persistence.
...
- Local deployment automation now includes a first-party Dockerfile and Compose stack for Postgres, Redis, MinIO, API, scheduler worker, audio worker, analytics worker, export worker, health checks, migrations, demo seeding, and a shared durable object-storage volume.
