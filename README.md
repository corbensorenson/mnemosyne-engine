...
Morning Forge, GraphFeed, first-party Paced Read, WalkMode, Evening Lock-In, and SleepCue sync post backend-compatible queued responses directly to the first-party completion endpoints when API sync is configured, with the offline receipt route retained for legacy and receipt-only payloads.
