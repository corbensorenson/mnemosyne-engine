...
Backend-compatible Morning Forge, GraphFeed, first-party Paced Read, WalkMode, Evening Lock-In, SleepCue, wearable sleep, and incident report queue items sync directly to `POST /api/morning-forge/complete`, `POST /api/watch-packets/:id/complete`, `POST /api/paced-read/complete`, `POST /api/walk-mode/complete`, `POST /api/evening-lock-in/complete`, `POST /api/sleep/playback/events`, `POST /api/sleep/recall/complete`, `POST /api/wearables/sync`, and `POST /api/ops/incidents/reports` from the PWA transport; older receipt-only payloads continue to use `/api/offline/actions/sync`.
