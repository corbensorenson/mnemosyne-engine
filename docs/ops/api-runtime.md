...
Backend-compatible Morning Forge, WalkMode, and Evening Lock-In queue items sync directly to `POST /api/morning-forge/complete`, `POST /api/walk-mode/complete`, and `POST /api/evening-lock-in/complete` from the PWA transport; older receipt-only payloads continue to use `/api/offline/actions/sync`.
