...
Backend-compatible Morning Forge, GraphFeed, first-party Paced Read, WalkMode, Evening Lock-In, SleepCue, and wearable sleep queue items sync directly to `POST /api/morning-forge/complete`, `POST /api/watch-packets/:id/complete`, `POST /api/paced-read/complete`, `POST /api/walk-mode/complete`, `POST /api/evening-lock-in/complete`, `POST /api/sleep/playback/events`, `POST /api/sleep/recall/complete`, and `POST /api/wearables/sync` from the PWA transport; older receipt-only payloads continue to use `/api/offline/actions/sync`.
