...
The first executable worker handles `scheduler:generate_daily_packet`, `audio_render:render_sleep_audio`, and `export:build_privacy_export`. A scheduler job persists the daily packet, sleep packet, and audio plan, then queues the audio render job. The audio worker writes a deterministic render-manifest object through configured object storage and updates the audio plan to `ready`. The export worker builds the user data export bundle and writes a JSON artifact into the first-party `export` bucket.
