...
When `VITE_MNEMOSYNE_API_URL` is configured, the browser transport posts backend-compatible learning and health actions directly to their first-party domain endpoints: Morning Forge, GraphFeed, Paced Read, WalkMode, Evening Lock-In, SleepCue playback, SleepCue recall, and wearable sleep sync. Older receipt-only payloads still go to `POST /api/offline/actions/sync`, where the API validates the queue item envelope, requires the owning user to exist, writes an `offline_action_synced` audit event, and returns a receipt id.
