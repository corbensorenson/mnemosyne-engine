...
When `VITE_MNEMOSYNE_API_URL` is configured, the browser transport posts backend-compatible learning, health, privacy, and ops actions directly to their first-party domain endpoints: Morning Forge, Tutor, GraphFeed, Paced Read, SpeedListen, WalkMode, Evening Lock-In, SleepCue playback, SleepCue recall, wearable sleep sync, privacy export jobs, scoped privacy deletion, and incident report creation. Older receipt-only payloads still go to `POST /api/offline/actions/sync`, where the API validates the queue item envelope, requires the owning user to exist, writes an `offline_action_synced` audit event, and returns a receipt id.
